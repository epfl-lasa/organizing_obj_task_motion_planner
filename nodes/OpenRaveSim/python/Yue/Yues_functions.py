...
def closeFingerExecuteGrasp(robot, objToGraspIndex, objIndexRemainingExceptObjInFocus, objIndexNotRemaining, objectList, env):
	# the settings for the barrett hand	
	initDOF = robot.GetDOFValues()
	objToGrasp = objectList[objToGraspIndex]
	maxJointValue = robot.GetDOFLimits()[1][:]
	controlJointsLevelTotal = [7, 8, 9]
	movableVecLevelTotal = [True, True, True]
	incremNumber = 10
	if (objToGrasp.GetName() != 'kitchen_knife') and (objToGrasp.GetName() != 'fork'):
		incremNumber = 20
	while(True):
		for i_joint in range(shape(controlJointsLevelTotal)[0]):
			for i_increm in range(10):
				robot.SetDOFValues([(i_increm) * maxJointValue[controlJointsLevelTotal[i_joint]] /10.0],[controlJointsLevelTotal[i_joint]])
				...
				if env.CheckCollision(robot, objToGrasp): # collision with object to be grasped
					# pdb.set_trace() # breakpoint
					robot.SetDOFValues([(i_increm-1) * maxJointValue[controlJointsLevelTotal[i_joint]] /10.0],[controlJointsLevelTotal[i_joint]])
					movableVecLevelTotal[i_joint] = False
					break
				if i_increm == 9:
					movableVecLevelTotal[i_joint] = False
		# pdb.set_trace() # breakpoint
	...
	"""
	# the settings for the allegro hand
...
def executeCloseFingerGrasp(robot, objToGraspIndex, objectList, executeMode, env):
	# the settings for the barrett hand
	...
	maxJointValue = robot.GetDOFLimits()[1][:]
	controlJointsLevelTotal = [7,8,9]
	graspingJointValue = [5,5,5,5,5,5,5]
	movableVecLevelTotal = [True, True, True]
	diffNum = 30.0
	while(True):
		if executeMode == 'animation':
			time.sleep(0.5)
		for i_joint in range(shape(controlJointsLevelTotal)[0]):
			for i_increm in range(int(diffNum)):
				robot.SetDOFValues([(i_increm) * maxJointValue[controlJointsLevelTotal[i_joint]]/diffNum],[controlJointsLevelTotal[i_joint]])
				# pdb.set_trace() #breakpoint
				if env.CheckCollision(robot, objToGrasp): # collision with object to be grasped
					graspingJointValue[i_joint+1] = robot.GetDOFValues()[i_joint + 7]
					robot.SetDOFValues([(i_increm-1) * maxJointValue[controlJointsLevelTotal[i_joint]]/diffNum],[controlJointsLevelTotal[i_joint]])
					movableVecLevelTotal[i_joint] = False
					break
				if i_increm == 9:
					movableVecLevelTotal[i_joint] = False
		if not np.any(movableVecLevelTotal):
			break
	# pdb.set_trace() #breakpoint
	return graspingJointValue
# robot.SetDOFValues([4 * maxJointValue[controlJointsLevelTotal[2]] /10.0],[controlJointsLevelTotal[2]])
				# robot.SetDOFValues([0.5],[10])
...
def executeOpenFingerAfterGrasp(robot, initDOF, executeMode, env):
	# setting for barrett hand
	crrtDOFTotal = robot.GetDOFValues()
	crrtDOF = robot.GetDOFValues()[7:10]
...
def transferMotion(objInFocusIndex, finalCfgObjFocus, objIndexNotRemaining, objectList, area, robot, transitTransferRRTGraphParameter, env):
	...
	for i_grasp in range(numGrasp):
		...
		# pdb.set_trace()
		# h4 = [], h4.append(PlotFrame(env, graspTransform[i_grasp], 0.05))
		if sol_DOF is None:
			if i_grasp == (numGrasp -1):
...
def intermediatePose(robot, objIndex, objIndexRemaining, objectList, area, totalPath, transitTransferRRTGraphParameter, env):
	...
	# pdb.set_trace() #breakpoint	
	robot.SetDOFValues(initDOF)
	if (intermediatePosePath[2] == True) and (intermediatePosePath[3] == []):
		pdb.set_trace() #breakpoint	
...
def pathExecutionWithArm(totalPath, objectList, robot, env, transitTransferRRTGraphParameter, executeMode, area):
	pathWithArm = []
	manip = robot.SetActiveManipulator("lwr")
	setObjListTransformFromCfgList(totalPath[0][0], objectList, area)
	pathNumber = shape(totalPath)[0]
	...
	# pathForClosingFingers = [5,5,5,5,5,5,5]
	pathForOpeningFingers = [10,10,10,10,10,10,10]
	for i_path in range(pathNumber):
		...
		# pdb.set_trace() # breakpoint
		reachingPath = executeTransitTransferRRT(startDOFArm, grasp1DOF, [], robot, [], objectListExecute, transitTransferRRTGraphParameter, area, executeMode, env)
		concatenatePathToTotal(pathWithArm, reachingPath)
		# grasp
		pathForClosingFingers = executeCloseFingerGrasp(robot, objMovedIndex, objectListExecute, executeMode, env)
		concatenatePathToTotal(pathWithArm, pathForClosingFingers)
		# move object to target grasping position
		movingPath = executeTransitTransferRRT(grasp1DOF, grasp2DOF, rrtPath, robot, objectToMove, objectListExecute, transitTransferRRTGraphParameter, area, executeMode, env)
		concatenatePathToTotal(pathWithArm, movingPath)
		# open
		executeOpenFingerAfterGrasp(robot, initDOFHand, executeMode, env)
		concatenatePathToTotal(pathWithArm, pathForOpeningFingers)
	reachingToInit = executeTransitTransferRRT(grasp2DOF, initDOFArm, [], robot, [], objectListExecute, transitTransferRRTGraphParameter, area, executeMode, env)
	concatenatePathToTotal(pathWithArm, pathForOpeningFingers)
	return pathWithArm

def concatenatePathToTotal(total, newPath):
	# pdb.set_trace() # breakpoint
	dimNewPath = shape(shape(newPath))[0]
	if dimNewPath == 1:
		total.append(newPath)
	elif dimNewPath == 2:
		lengthNewPath = shape(newPath)[0]
		for i_config in range(lengthNewPath):
			total.append(newPath[i_config])
	elif dimNewPath == 3:
		lengthNewPath = shape(newPath)[0]
		for i_config in range(lengthNewPath):
			total.append(newPath[i_config][0])
	else:
		pdb.set_trace() # breakpoint


def totalPathExecutionWithArm(pathFoundInGraph, reArrangeGraph, objectList, robot1, area, transitTransferRRTGraphParameter, executeMode, env):
	totalPathWithArm = []
	manip = robot1.SetActiveManipulator("lwr")
	initDOFArm = robot1.GetDOFValues(manip.GetArmIndices())
	pathForFinishingManipulation = [15,15,15,15,15,15,15]
	objectListExecute = objectList[:]
	objectListExecute.append(env.GetBodies()[2])
	for i_edge in range(shape(pathFoundInGraph)[0]-1):
		...
		pathWithArm = pathExecutionWithArm(PLNMRPath, objectList, robot1, env, transitTransferRRTGraphParameter, executeMode, area)
		concatenatePathToTotal(totalPathWithArm, pathWithArm)
	startDOFArm = robot1.GetDOFValues(manip.GetArmIndices())
	finalPathWithArm = executeTransitTransferRRT(startDOFArm, initDOFArm, [], robot1, [], objectListExecute, transitTransferRRTGraphParameter, area, executeMode, env)
	concatenatePathToTotal(totalPathWithArm, finalPathWithArm)
	concatenatePathToTotal(totalPathWithArm, pathForFinishingManipulation)
	# pdb.set_trace()
...
def	executeTransitTransferRRT(startingNodeDOF, targetNodeDOF, rrtPath, robot, objGrasped, cllsnCheckObjList, transitTransferRRTGraphParameter, area, executeMode, env):
	if rrtPath == []:
	...
	# pdb.set_trace() # breakpoint
	if executeMode == 'animation':
		runPathRRT(robot, objGrasped, smoothedPath, maxDOFIncrementInDemo = 0.1, env=env)
	else:
		# targetNodeDOF
		manip = robot.SetActiveManipulator("lwr")
		endTool = manip.GetEndEffector()
...
def runPathRRT(robot, objGrasped, path, maxDOFIncrementInDemo, env):
	manip = robot.SetActiveManipulator("lwr")
	endTool = manip.GetEndEffector()
	if objGrasped != []:
		relativeMatrixRobotToObjGrasped = dot(inv(endTool.GetTransform()), objGrasped.GetTransform())
...
def giveGraspTransformsFromObject(objectInFocus, env):
	...
	if objectInFocus.GetName() == 'fork' or objectInFocus.GetName() == 'kitchen_knife':
		...
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('z', -pi/5))
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('y', pi))
		...
		# grasp 2
		graspTransform2 = dot(graspTransform1, giveRotationMatrix3D_4X4('z', pi))
		...
		# pdb.set_trace() # breakpoint
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('x', pi))
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0,0, -0.322]) # -0.272
		genGraspInCircle(totalGrasps = graspTransform, inputGrasp = graspTransform1, numGrasp = 32, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z')
	elif objectInFocus.GetName() == 'kitchen_cup':
		# pdb.set_trace() # breakpoint
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('x', pi))
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0,0, -0.23]) # -0.22
		genGraspInCircle(totalGrasps = graspTransform, inputGrasp = graspTransform1, numGrasp = 32, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z')
		# pdb.set_trace() #breakpoint
		...
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('x', pi))
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0, 0, -0.25]) # -0.24
		genGraspInCircle(totalGrasps = graspTransform, inputGrasp = graspTransform1, numGrasp = 32, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z')
		# pdb.set_trace() #breakpoint
	
	# elif objectInFocus.GetName() == 'domino_sugar':
	
	elif objectInFocus.GetName() == 'red_metal_bowl':
		deltaZ = 0.215
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0, 0, deltaZ])
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('y', -pi))
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('z', -pi))
		genGraspInCircle(totalGrasps = graspTransform, inputGrasp = graspTransform1, numGrasp = 32, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z')
	
	elif objectInFocus.GetName() == 'red_metal_cup':
		deltaZ = 0.23
		graspTransform2 = []
		graspTransformGroup1 = []
		graspTransformGroup2 = []
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0, 0, deltaZ])
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('y', -pi))
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('z', pi/4))
		genGraspInArc(totalGrasps = graspTransformGroup1, inputGrasp = graspTransform1, numGrasp = 10, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z', angle = 11.0/18*pi)
		graspTransform2 = dot(graspTransform1, giveRotationMatrix3D_4X4('z', 8.0/9*pi))	
		genGraspInArc(totalGrasps = graspTransformGroup2, inputGrasp = graspTransform2, numGrasp = 10, radius = 0, radiusOffsetFromAxis = 'x', distributionAxis = 'z', angle = 11.0/18*pi)
		graspTransform = combineTwoGraspGroups(graspTransformGroup1, graspTransformGroup2)

	elif objectInFocus.GetName() == 'master_chef_coffee':
		deltaZ = 0.28
		graspTransform1 = objectInFocus.GetTransform()
		graspTransform1 = executeTranslationFromCrrtFrame(graspTransform1, [0, 0, deltaZ])
		graspTransform1 = dot(graspTransform1, giveRotationMatrix3D_4X4('y', -pi))
...
def	genGraspInArc(totalGrasps, inputGrasp, numGrasp, radius, radiusOffsetFromAxis, distributionAxis, angle):
	# the angle is the angle spanned by both start and end grasps
	if radiusOffsetFromAxis == 'x':
		offSetGrasp = [radius, 0, 0]
	elif radiusOffsetFromAxis == 'y':
		offSetGrasp = [0, radius, 0]
		...
		offSetGrasp = [0, 0, radius]
	diffRadian = angle/(numGrasp-1)
	for i_grasp in range(numGrasp):
		intermediateTransform = dot(inputGrasp,  giveRotationMatrix3D_4X4(distributionAxis, diffRadian * i_grasp))
		totalGrasps.append(executeTranslationFromCrrtFrame(intermediateTransform, offSetGrasp))

def	combineTwoGraspGroups(graspTransformGroup1, graspTransformGroup2):
	graspNum = shape(graspTransformGroup2)[0]
	for i_grasp in range(graspNum):
		graspTransformGroup1.append(graspTransformGroup2[i_grasp])
	return graspTransformGroup1
